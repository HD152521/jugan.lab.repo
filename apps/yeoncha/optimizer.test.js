...
test('recommend: 설·추석 빼기는 dedup 이전에 적용돼 대안이 사라지지 않는다', () => {
  // 큰 limit으로 정렬 절단 영향 배제
  const withM = recommend(calendar, 2, 1000);
  const noM = recommend(calendar, 2, 1000, { excludeMyeongjeol: true });
  // 제외 시 명절 구간이 하나도 없어야
  assert.equal(noM.some(isMyeongjeol), false);
  // 사후 필터(버그 방식)보다 결과가 줄지 않아야 — 삼켜졌던 비명절 대안이 살아남음
  const buggyPostFilter = withM.filter((s) => !isMyeongjeol(s));
  assert.ok(noM.length >= buggyPostFilter.length);
  // 명절 구간과 겹치는 자리에 비명절 대안이 실제로 최소 1개 복구됨
  const mj = withM.find(isMyeongjeol);
  if (mj) {
    const hasAlt = noM.some((s) => s.start <= mj.end && mj.start <= s.end);
    assert.ok(hasAlt, '명절 자리에 비명절 대안이 있어야 한다');
  }
});
